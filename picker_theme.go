...
// pickerTheme holds SGR prefixes for picker chrome, resolved from Herdr's
...
// LoadPickerTheme resolves the picker chrome palette from Herdr's config.
// Missing or unparsable config falls back to Herdr's default palette.
func LoadPickerTheme(configPath string) (pickerTheme, []string) {
	if configPath == "" {
	...
	if err != nil {
		if os.IsNotExist(err) {
			return resolvePickerTheme("", nil), nil
		}
		return resolvePickerTheme("", nil), []string{fmt.Sprintf("hseh theme: read %s: %v", configPath, err)}
	}
	var file herdrThemeFile
	if err := toml.Unmarshal(payload, &file); err != nil {
		return resolvePickerTheme("", nil), []string{fmt.Sprintf("hseh theme: parse %s: %v", configPath, err)}
	}
	return resolvePickerTheme(file.Theme.Name, file.Theme.Custom), nil
}
...
// terminalPickerTheme uses the terminal's own 16-color palette, matching Herdr's "terminal" theme.
func terminalPickerTheme() pickerTheme {
	return pickerTheme{
		Name:      "terminal",
		Accent:    "\x1b[34m",
		TabActive: "\x1b[7;34m",
...
func defaultPickerTheme() pickerTheme {
	return resolvePickerTheme(defaultHerdrThemeName, nil)
}
...
func (m pickerModel) th() pickerTheme {
	if m.theme.Name == "" {
		return defaultPickerTheme()
	}
	return m.theme
}
