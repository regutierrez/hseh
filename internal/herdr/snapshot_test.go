...
func TestFocusHerdrAgentContextSyncsReturnedTab(t *testing.T) {
		...
		for {
			...
			go func(c net.Conn) {
				...
				case "agent.focus":
					...
					mu.Unlock()
					agent := AgentRow{PaneRow: PaneRow{
						PaneID:      "w1:p2",
						TabID:       "w1:t2",
						WorkspaceID: "w1",
						Agent:       "pi",
					}}
					result = AgentFocusEnvelope{Type: "agent_info", Agent: &agent}
				case "tab.focus":
					var params struct {
	...
	if err := FocusAgentContext(context.Background(), "w1:p2"); err != nil {
		t.Fatal(err)
	}
	mu.Lock()
	defer mu.Unlock()
...
func TestFocusHerdrAgentContextRejectsMissingTabID(t *testing.T) {
	...
	go func() {
		for {
			...
			go func(c net.Conn) {
				...
				methods = append(methods, req.Method)
				mu.Unlock()
				agent := AgentRow{PaneRow: PaneRow{PaneID: "w1:p2", WorkspaceID: "w1", Agent: "pi"}}
				result := any(AgentFocusEnvelope{Type: "agent_info", Agent: &agent})
				payload, _ := json.Marshal(map[string]any{"id": req.ID, "result": result})
				_, _ = c.Write(append(payload, '\n'))
			}(conn)
		}
	...
	t.Setenv("HERDR_SOCKET_PATH", socketPath)
	err = FocusAgentContext(context.Background(), "w1:p2")
