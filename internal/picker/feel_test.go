...
func TestFirstSnapshotPreselectsDoneAgentOverRecentlyUsed(t *testing.T) {
	m := newAsyncModel("agents", terminalTheme(), 0, 100, nil)
	m.width, m.height = 120, 30
	m.snapshotSeq, m.snapshotInFlight = 1, true
	history := focus.EmptyHistory(herdr.ContinuityWitness{})
	...
	got := next.(model)
	if !got.snapshotReady || got.selectedID != selectionID(KindAgent, "w1:p1#1") {
		t.Fatalf("first snapshot did not preselect done agent: %q", got.selectedID)
	}
}
