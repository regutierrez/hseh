...
func TestOpenReusableSpaceCancelBeforeMutation(t *testing.T) {
	...
	h := &hsehtest.Server{Snapshot: herdr.SessionSnapshot{Version: "0.9.0"}, SnapshotDelay: 400 * time.Millisecond}
...
func TestOpenReusableSpaceReportsSubmittedCountWhenLaterCommandFails(t *testing.T) {
	...
	h := &hsehtest.Server{Snapshot: herdr.SessionSnapshot{Version: "0.9.0"}, FailMethod: "pane.send_input", FailAfter: 2}
	socket, state := hsehtest.Start(t, h)
	t.Setenv("HERDR_SOCKET_PATH", socket)
	t.Setenv("HERDR_PLUGIN_STATE_DIR", state)
	t.Setenv("HERDR_PLUGIN_CONFIG_DIR", config)
