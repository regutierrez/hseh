...
func TestPromptWorkingDirRejected(t *testing.T) {
	def := Definition{Name: "x", WorkingDir: "{prompt}", Tabs: []DefinitionTab{{Name: "t"}}, SourceFile: "x.toml"}
...
func TestReplaceSpaceDefinitionFileDetectsConcurrentEdit(t *testing.T) {
	...
	err := replaceDefinitionFile(path, original, next)
